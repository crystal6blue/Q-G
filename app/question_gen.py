    ...
    *,
    ...
    def normalize_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            elif len(variants) < variant_count:
                start = len(variants) + 1
                variants.extend([f"Option {i}" for i in range(start, variant_count + 1)])
        ...
        if items is None:
            ...
            for line in lines:
                ...
                items.append(
                    {
                        "question": q,
                        "variants": ["True", "False"] + [f"Option {i}" for i in range(3, variant_count + 1)],
                        "correct_answers": [0],
                    }
                )
    ...
    if len(normalized) > n:
        ...
        # Final safety fallback for strict API shape.
        next_num = len(normalized) + 1
        while len(normalized) < n:
            variants = [f"Option {i}" for i in range(1, variant_count + 1)]
            normalized.append(
                {
                    "question": f"Generated question {next_num}",
                    "variants": variants,
                    "correct_answer": variants[0],
                }
            )
            next_num += 1
